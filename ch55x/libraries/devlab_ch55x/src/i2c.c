...
//
// PIN_SDA and PIN_SCL are defined in I2C_config.h:
// PIN_SDA - pin connected to serial data of the I2C bus (default: P1.6)
// PIN_SCL - pin connected to serial clock of the I2C bus (default: P1.7)
...
// I2C probe for device at address (returns 1 if device responds with ACK, 0 if NACK)
uint8_t I2C_probe(uint8_t addr) {
  ...
  // Read ACK bit
  I2C_SDA_HIGH();                           // release SDA to read ACK
  I2C_DELAY_H();
  I2C_DELAY_L();
  I2C_SCL_HIGH();                           // clock HIGH
  I2C_DELAY_H();
  ack = !I2C_SDA_READ();                    // read ACK (LOW = ACK, HIGH = NACK)
  I2C_SCL_LOW();
